...
export const PrimaryButton = ({ title, onClick, full, type, disabled,form }: PrimaryButtonProps) => {
    return (
        <button
            ...
            disabled={disabled}
            onClick={onClick}
            className={`bg-primary-shadow text-sm xl:text-base hover:bg-primary text-primary-darker hover:text-white md:px-3 lg:px-5 py-2 rounded-xl font-semibold transition-colors shadow-sm cursor-pointer
                        ${full ? "w-full" : "w-full md:w-auto"}
            `}
        >
            {title}
